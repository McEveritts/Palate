...
const formatCooklangForEyeView = (text: string) => {
  if (!text) return "";
  return text
    // Fix hallucinated paths generated by the AI
    .replace(/@Documents\\[^\s]+\\([^\\]+)\.md/g, '@$1')
    // Match @ingredient{amount%unit}
    .replace(/@([^\{]+)\{([^%\}]+)%([^}]+)\}/g, '**$2 $3 $1**')
    // Match @ingredient{amount}
    .replace(/@([^\{]+)\{([^}]+)\}/g, '**$2 $1**')
    // Match @ingredient without braces
    .replace(/@([a-zA-Z0-9_-]+)/g, '**$1**')
    // Match #cookware{}
    .replace(/#([^\{]+)\{[^}]*\}/g, '**$1**')
    // Match #cookware without braces
    .replace(/#([a-zA-Z0-9_-]+)/g, '**$1**');
};
                  ...
                  <div className="bg-black/20 p-8 md:p-10 rounded-2xl border border-white/5 shadow-inner">
                    {isRawView ? (
                      ...
                      <div className="prose prose-invert prose-lg prose-indigo max-w-none">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>
                          {formatCooklangForEyeView(selectedRecipe.content)}
                        </ReactMarkdown>
                      </div>
                    )}
                  </div>
