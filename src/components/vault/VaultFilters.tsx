...
interface VaultFiltersProps {
  ...
  activeCategory: 'all' | 'mains' | 'sides' | 'appetizers' | 'desserts';
  setActiveCategory: (c: 'all' | 'mains' | 'sides' | 'appetizers' | 'desserts') => void;
}
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 mb-8">
      ...
      <div className="flex bg-black/40 p-1 rounded-xl border border-white/10 backdrop-blur-xl shrink-0">
        {(['all', 'mains', 'sides', 'appetizers', 'desserts'] as const).map((cat) => (
          <button
            key={cat}
